// /pages/api/dispatch-icabbi.js
// Calculate driver arrival time using iCabbi's real driver availability
// - Checks actual available drivers in iCabbi system
// - Uses Google Distance Matrix for traffic-aware routing
// - Formats arrival time in America/Denver (DST-aware)
// - Returns friendly eta_summary buckets with driver info

export default async function handler(req, res) {
  // --- CORS ---
  const ALLOW_ORIGIN = "*";
  if (req.method === "OPTIONS") {
    res.setHeader("Access-Control-Allow-Origin", ALLOW_ORIGIN);
    res.setHeader("Access-Control-Allow-Methods", "POST, OPTIONS");
    res.setHeader("Access-Control-Allow-Headers", "Content-Type, Authorization");
    return res.status(200).end();
  }
  res.setHeader("Access-Control-Allow-Origin", ALLOW_ORIGIN);

  if (req.method !== "POST") {
    return res.status(405).json({ ok: false, error: "METHOD_NOT_ALLOWED" });
  }

  try {
    const googleKey = process.env.GOOGLE_MAPS_API_KEY;
    const icabbiAppKey = process.env.ICABBI_APP_KEY;
    const icabbiSecretKey = process.env.ICABBI_SECRET_KEY;
    const icabbiBaseUrl = process.env.ICABBI_BASE_URL || 'https://api.icabbi.com/v2';

    if (!googleKey) {
      return res.status(500).json({ ok: false, error: "NO_GOOGLE_MAPS_API_KEY" });
    }

    if (!icabbiAppKey || !icabbiSecretKey) {
      console.warn('[dispatch-icabbi] iCabbi credentials not configured, falling back to base location');
    }

    // --- Parse body & coerce types ---
    const { pickup_lat, pickup_lng, pickup_address } = req.body || {};

    const pLat = typeof pickup_lat === "string" ? parseFloat(pickup_lat) : pickup_lat;
    const pLng = typeof pickup_lng === "string" ? parseFloat(pickup_lng) : pickup_lng;

    if (
      pLat === undefined ||
      pLng === undefined ||
      Number.isNaN(pLat) ||
      Number.isNaN(pLng)
    ) {
      return res.status(400).json({
        ok: false,
        error: "MISSING_COORDS",
        details: "pickup_lat and pickup_lng required",
      });
    }

    if (Math.abs(pLat) > 90 || Math.abs(pLng) > 180) {
      return res.status(400).json({
        ok: false,
        error: "INVALID_COORDS",
        details: "Coordinates are out of valid range",
      });
    }

    // High Mountain Taxi base (fallback if no drivers available)
    const HM_BASE = {
      lat: 39.2228,
      lng: -106.8692,
      address: "214 Aspen Airport Business Ctr, Unit B, Aspen, CO 81611",
    };

    let closestDriverLocation = null;
    let closestDriverDistance = null;
    let driversAvailable = 0;

    // --- Try to get actual available drivers from iCabbi ---
    if (icabbiAppKey && icabbiSecretKey) {
      try {
        console.log('[dispatch-icabbi] Checking iCabbi for available drivers');
        
        const icabbiResponse = await fetch(`${icabbiBaseUrl}/driver/available`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'X-App-Key': icabbiAppKey,
            'X-Secret-Key': icabbiSecretKey
          },
          body: JSON.stringify({
            lat: pLat,
            lng: pLng,
            radius: 15000 // 15km radius
          })
        });

        if (icabbiResponse.ok) {
          const icabbiData = await icabbiResponse.json();
          const drivers = icabbiData.body?.drivers || [];
          driversAvailable = drivers.length;

          console.log(`[dispatch-icabbi] Found ${driversAvailable} available drivers`);

          if (drivers.length > 0) {
            // Use closest available driver
            const closestDriver = drivers[0];
            closestDriverLocation = {
              lat: parseFloat(closestDriver.lat),
              lng: parseFloat(closestDriver.lng),
              driver_id: closestDriver.id,
              driver_name: closestDriver.name,
              vehicle_number: closestDriver.vehicle_number
            };
            closestDriverDistance = closestDriver.distance; // meters from iCabbi

            console.log('[dispatch-icabbi] Using closest driver:', closestDriver.name);
          }
        } else {
          console.warn('[dispatch-icabbi] iCabbi API returned error:', icabbiResponse.status);
        }
      } catch (icabbiErr) {
        console.error('[dispatch-icabbi] Error fetching drivers from iCabbi:', icabbiErr);
        // Continue with fallback
      }
    }

    // If no drivers available, use base location
    const dispatchLocation = closestDriverLocation || HM_BASE;
    const dispatchAddress = closestDriverLocation 
      ? `Driver ${closestDriverLocation.driver_name}` 
      : HM_BASE.address;

    // --- Google Distance Matrix (traffic-aware) ---
    const params = new URLSearchParams({
      origins: `${dispatchLocation.lat},${dispatchLocation.lng}`,
      destinations: `${pLat},${pLng}`,
      units: "imperial",
      mode: "driving",
      region: "US",
      departure_time: "now",
      traffic_model: "best_guess",
      key: googleKey,
    });

    const url = `https://maps.googleapis.com/maps/api/distancematrix/json?${params.toString()}`;
    const gRes = await fetch(url);
    const data = await gRes.json();

    if (data?.status !== "OK" || !data?.rows?.[0]?.elements?.[0]) {
      return res.status(502).json({
        ok: false,
        error: "GOOGLE_API_ERROR",
        google_status: data?.status || "NO_STATUS",
        details: "Failed to retrieve route data from Google Maps",
      });
    }

    const element = data.rows[0].elements[0];
    if (element?.status !== "OK") {
      return res.status(200).json({
        ok: false,
        error: "ROUTE_NOT_FOUND",
        element_status: element?.status || "NO_ELEMENT",
        details: "No valid route found from driver to pickup",
      });
    }

    const durationSeconds =
      element.duration_in_traffic?.value ?? element.duration?.value ?? 0;
    const durationMinutes = Math.ceil(durationSeconds / 60);

    const distanceMeters = element.distance?.value ?? 0;
    const distanceMilesNum = distanceMeters / 1609.344;
    const distanceMiles = Math.round(distanceMilesNum * 10) / 10;
    const distanceText = element.distance?.text || `${distanceMiles.toFixed(1)} mi`;

    // Add prep time for driver
    const prepTimeMinutes = 2;
    const totalEtaMinutes = durationMinutes + prepTimeMinutes;

    // --- Time formatting in America/Denver ---
    const ASPEN_TZ = "America/Denver";
    const nowUtcMs = Date.now();
    const arrivalUtcMs = nowUtcMs + totalEtaMinutes * 60 * 1000;

    const fmtLocalTime = (ms, tz = ASPEN_TZ) =>
      new Intl.DateTimeFormat("en-US", {
        hour: "numeric",
        minute: "2-digit",
        hour12: true,
        timeZone: tz,
      }).format(ms);

    const estimated_arrival_time = fmtLocalTime(arrivalUtcMs, ASPEN_TZ);
    const now_local_time = fmtLocalTime(nowUtcMs, ASPEN_TZ);

    // --- Friendly ETA summary buckets ---
    let etaSummary;
    if (totalEtaMinutes <= 5) {
      etaSummary = "about 5 minutes";
    } else if (totalEtaMinutes <= 10) {
      etaSummary = "about 10 minutes";
    } else if (totalEtaMinutes <= 15) {
      etaSummary = "about 10-15 minutes";
    } else if (totalEtaMinutes <= 20) {
      etaSummary = "about 15-20 minutes";
    } else if (totalEtaMinutes <= 30) {
      etaSummary = "about 20-30 minutes";
    } else {
      etaSummary = `about ${totalEtaMinutes} minutes`;
    }

    // --- Build response ---
    const nowIsoUtc = new Date(nowUtcMs).toISOString();
    const arrivalIsoUtc = new Date(arrivalUtcMs).toISOString();

    return res.status(200).json({
      ok: true,

      // Dispatch info
      dispatch_from: dispatchAddress,
      dispatch_lat: dispatchLocation.lat,
      dispatch_lng: dispatchLocation.lng,
      driver_info: closestDriverLocation || null,
      drivers_available: driversAvailable,

      // Pickup info
      pickup_address: pickup_address || null,
      pickup_lat: pLat,
      pickup_lng: pLng,

      // Metrics
      drive_time_minutes: durationMinutes,
      prep_time_minutes: prepTimeMinutes,
      total_eta_minutes: totalEtaMinutes,

      // Distance
      distance_miles: distanceMiles,
      distance_text: distanceText,

      // Times
      timezone: ASPEN_TZ,
      now_local_time,
      estimated_arrival_time,
      now_utc_iso: nowIsoUtc,
      arrival_utc_iso: arrivalIsoUtc,

      // Voice-friendly summary
      eta_summary: etaSummary,

      // Capacity warning
      capacity_warning: driversAvailable === 0 
        ? "Running lean right now, might take a bit longer" 
        : null,

      // Metadata
      calculated_at: nowIsoUtc,
      source: closestDriverLocation ? 'icabbi_driver' : 'base_location'
    });
  } catch (error) {
    console.error("[dispatch-icabbi] error:", error);
    return res.status(500).json({
      ok: false,
      error: "DISPATCH_ETA_FAILED",
      message: error?.message,
    });
  }
}
